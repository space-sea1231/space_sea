...
namespace PST {
    ...
    int Query(int l, int r, int pos, int p, int q) {
        if (l == r) return l;
        if (2 * (node[node[q].l].cnt - node[node[p].l].cnt) > pos) return Query(l, mid, pos, node[p].l, node[q].l);
        if (2 * (node[node[q].r].cnt - node[node[p].r].cnt) > pos) return Query(mid + 1, r, pos, node[p].r, node[q].r);
        return 0;
    }
} using namespace PST;
...
signed main() {
    ...
    for (int i = 1; i <= m; i++) {
        int l, r;
        cin >> l >> r;
        printf("%d\n", b[Query(1, n, r - l + 1, root[l - 1], root[r])]);
    }
    return 0;
}