...
namespace SegmentTree {
    ...
    void Down(int x, int l, int r) {
        ...
        if (node[x].vis == 1) {
            if (node[x << 1].vis < 2) node[x << 1].vis ^= 1;
                ...
                if (l != mid) Down(x << 1, l, mid), Up(x << 1, l, mid);
                node[x << 1].vis = 1;    
            }
            if (node[x << 1 | 1].vis < 2) node[x << 1 | 1].vis ^= 1;
            else {
                if (r != mid + 1) Down(x << 1 | 1, mid + 1, r), Up(x << 1 | 1, mid + 1, r);
                node[x << 1 | 1].vis = 1;    
            }
    ...
    int Query(int x, int l, int r, int L, int R) {
        // if (flag) printf("Debug7:l=%d r=%d sum=%d %d\n", l, r, node[x].sum, node[5].l_zero);
        if (L <= l && r <= R)  {
            // #ifdef __Debug
    ...
    void Out(int x, int l, int r) {
        if (l == r) {
            printf("pos:%d L-1=%d R-1=%d L-0=%d R-0=%d vis=%d sum=%d\n", l, node[x].l_one, node[x].r_one, node[x].l_zero, node[x].r_zero, node[x].vis, node[x].sum);
            return;
        }
        Down(x, l, r);
        int mid = (l + r) >> 1;
