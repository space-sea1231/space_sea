...
int T, n, a[N], cnt, p[N], ans; bool vis[N];
...
signed main()
{
    ...
    {
        read(n), cnt = 0, ans = 1;
        for (rint i = 1; i <= (n << 1); ++ i) vis[i] = false;
        for (rint i = 1; i <= (n << 1); ++ i) read(a[i]), vis[a[i]] = true;
        for (rint i = 2; i <= (n << 1); i += 2)
            if (a[i] && !a[i - 1]) p[++ cnt] = a[i];
            else if (a[i - 1] && !a[i]) p[++ cnt] = a[i - 1];
        for (rint i = 1; i <= (n << 1); ++ i)
            if (!vis[i]) p[++ cnt] = i;
        sort(p + 1, p + cnt + 1); rint num1 = 0, num2 = 0;
        for (rint i = 1; i <= (n << 1); ++ i)
            if (!vis[i]) i <= p[cnt >> 1] ? ++ num1 : ++ num2;
        for (rint i = 2; i <= (n << 1); i += 2)
            if (a[i] && !a[i - 1])
            {
                if (a[i] <= p[cnt >> 1]) ans = 1ll * ans * (num2 --) % mod;
                else ans = 1ll * ans * (num1 --) % mod;
            }
            else if (!a[i] && a[i - 1])
            {
                if (a[i - 1] <= p[cnt >> 1]) ans = 1ll * ans * (num2 --) % mod;
                else ans = 1ll * ans * (num1 --) % mod;
            }
        while (num1) ans = 1ll * ans * num1 % mod * num1 % mod, Add(ans, ans), -- num1;
        print(ans, '\n');
    }
    return flush(), 0;
}